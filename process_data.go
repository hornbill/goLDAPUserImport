...
func processData() {
	logger(1, "Processing User Data\n", true)

	for user := range HornbillCache.UsersWorking {

		currentUser := HornbillCache.UsersWorking[user]
		//-- Current UserID
		userID := currentUser.Account.UserID
		hornbillUserData := HornbillCache.Users[userID]

		if userID == "" {
			continue
		}
		// Store What Jobs need doing
		var jobs = new(userImportJobs)

		//-- Check Map no need to loop
		if strings.ToLower(hornbillUserData.HUserID) == userID {

			jobs.update = checkUserNeedsUpdate(currentUser, hornbillUserData)

			jobs.updateProfile = checkUserNeedsProfileUpdate(currentUser, hornbillUserData)

			jobs.updateType = checkUserNeedsTypeUpdate(currentUser, hornbillUserData)

			jobs.updateSite = checkUserNeedsSiteUpdate(currentUser, hornbillUserData)

			jobs.updateImage = checkUserNeedsImageUpdate(currentUser, hornbillUserData)

			jobs.updateGroup = checkUserNeedsOrgUpdate(currentUser, hornbillUserData)
			...
			jobs.updateImage = checkUserNeedsImageCreate(currentUser, hornbillUserData)
			jobs.updateGroup = checkUserNeedsOrgCreate(currentUser, hornbillUserData)
			jobs.create = true
		}
		logger(2, "User: "+fmt.Sprintf("%s", userID)+" \n\tCreate: "+fmt.Sprintf("%t", jobs.create)+" \n\tUpdate: "+fmt.Sprintf("%t", jobs.update)+" \n\tUpdate Type: "+fmt.Sprintf("%t", jobs.updateType)+" \n\tUpdate Profile: "+fmt.Sprintf("%t", jobs.updateProfile)+" \n\tUpdate Site: "+fmt.Sprintf("%t", jobs.updateSite)+" \n\tRoles Count: "+fmt.Sprintf("%d", len(currentUser.Roles))+" \n\tUpdate Image: "+fmt.Sprintf("%t", jobs.updateImage)+" \n\tUpdate Group: "+fmt.Sprintf("%t", jobs.updateGroup)+"\n", true)
		//finaliseUser()
	}
	logger(1, "User Data Processed \n", true)
}
func checkUserNeedsOrgUpdate(importData *userWorkingDataStruct, currentData userAccountStruct) bool {

	if ldapImportConf.User.Org.Action == "Both" || ldapImportConf.User.Org.Action == "Update" {
		importData.GroupID = getOrgFromLookup(importData)
	}
	//-- Is User Already a Memeber of the Group
	for index := range HornbillCache.UserGroups[importData.Account.UserID] {
		if importData.GroupID == HornbillCache.UserGroups[importData.Account.UserID][index] {
			return false
		}
	}
	return true
}
func checkUserNeedsOrgCreate(importData *userWorkingDataStruct, currentData userAccountStruct) bool {

	if ldapImportConf.User.Org.Action == "Both" || ldapImportConf.User.Org.Action == "Create" {
		importData.GroupID = getOrgFromLookup(importData)
		return true
	}
	return false
...
func checkUserNeedsRoleUpdate(importData *userWorkingDataStruct, currentData userAccountStruct) {
	...
	if ldapImportConf.User.Role.Action == "Both" || ldapImportConf.User.Role.Action == "Update" {
		for index := range ldapImportConf.User.Role.Roles {
			...
			foundRole := false
			for index2 := range HornbillCache.UserRoles[importData.Account.UserID] {
				if roleName == HornbillCache.UserRoles[importData.Account.UserID][index2] {
					foundRole = true
				}
			}
			if !foundRole {
...
func setUserSiteValueForCreate(importData *userWorkingDataStruct, currentData userAccountStruct) bool {
	...
	if importData.Account.Site != currentData.HSite {
		return true
	}
	return false
}
...
func checkUserNeedsUpdate(importData *userWorkingDataStruct, currentData userAccountStruct) bool {
	if importData.Account.Name != currentData.HName {
		logger(1, "Name: "+fmt.Sprintf("%s", importData.Account.Name)+" - "+fmt.Sprintf("%s", currentData.HName), true)
		return true
	}
	if importData.Account.FirstName != currentData.HFirstName {
		logger(1, "FirstName: "+fmt.Sprintf("%s", importData.Account.FirstName), true)
		return true
	}
	if importData.Account.LastName != currentData.HLastName {
		logger(1, "LastName: "+fmt.Sprintf("%s", importData.Account.LastName), true)
		return true
	}
	if importData.Account.JobTitle != currentData.HJobTitle {
		logger(1, "JobTitle: "+fmt.Sprintf("%s", importData.Account.JobTitle), true)
		return true
	}
	if importData.Account.Phone != currentData.HPhone {
		logger(1, "Phone: "+fmt.Sprintf("%s", importData.Account.Phone)+" - "+fmt.Sprintf("%s", currentData.HPhone), true)
		return true
	}
	if importData.Account.Email != currentData.HEmail {
		logger(1, "Email: "+fmt.Sprintf("%s", importData.Account.Email), true)
		return true
	}
	if importData.Account.Mobile != currentData.HMobile {
		logger(1, "Mobile: "+fmt.Sprintf("%s", importData.Account.Mobile), true)
		return true
	}
	if importData.Account.AbsenceMessage != currentData.HAvailStatusMsg {
		logger(1, "AbsenceMessage: "+fmt.Sprintf("%s", importData.Account.AbsenceMessage), true)
		return true
	}
...
func checkUserNeedsProfileUpdate(importData *userWorkingDataStruct, currentData userAccountStruct) bool {
	...
	if importData.Profile.MiddleName != currentData.HMiddleName {
		logger(1, "MiddleName: "+fmt.Sprintf("%s", importData.Profile.MiddleName)+" - "+fmt.Sprintf("%s", currentData.HMiddleName), true)
		return true
	}

	if importData.Profile.JobDescription != currentData.HSummary {
